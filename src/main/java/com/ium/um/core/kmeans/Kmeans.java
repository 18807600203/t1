...
public class Kmeans<T> {  
     ...
     * 所有数据列表 
     */  
    private List<T> exGradValuesList = new ArrayList<T>();  
	...
	public Kmeans(List<T> list, int k) {  
        ...
        for (int i = 0; i < k; i++) {  
            initPlayers.add(exGradValuesList.get(i));  
        }  
    }  
    ...
    public List<T>[] comput() {  
        ...
        while (centerchange) {
            centerchange = false;  
            for (int i = 0; i < k; i++) {  
                results[i] = new ArrayList<T>();  //创建存放结果的数组。 分几类就创建几个
            }  
            for (int i = 0; i < exGradValuesList.size(); i++) {  
                T p = exGradValuesList.get(i);
